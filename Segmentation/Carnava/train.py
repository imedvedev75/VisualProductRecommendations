...
#DIR = "/nfs/users/alexey/Car"
#DIR = "/nfs/users/alexey/DRIVE/DRIVE_128"
#DIR = "/nfs/users/alexey/sh_hv"
    ...
    while True:
        for start in range(0, len(ids_train_split), batch_size):
            ...
            for id in ids_train_batch:
                img = cv2.imread(DIR + '/train/{}.jpg'.format(id))
                img = cv2.resize(img, (input_size, input_size))
                mask = cv2.imread(DIR + '/train/{}_mask.jpg'.format(id), cv2.IMREAD_GRAYSCALE)
...
callbacks = [EarlyStopping(monitor='val_loss',
             ...
             ModelCheckpoint(monitor='val_loss',
                             filepath=DIR + '/weights/best_weights.hdf5',
                             save_best_only=True,
                             save_weights_only=True),
             TensorBoard(log_dir=DIR + '/logs')]
